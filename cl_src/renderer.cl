...
float3	DiffLighting(struct RT_Data *RT_Data, float3 *orig,
	...
	if (true) {
		normal = get_mapped_normal(
			RT_Data->item + itemIndex, RT_Data->texture, normal, point);
	}
...
float3 GlowCalc(struct RT_Data *RT_Data, float3 *orig,
			float3 *dir, float tItem, float tIllu) {
	...
	for (int i = 0; i != RT_Data->illuNumber; i += 1) {
		...
		float3 illuVec = RT_Data->illu[i].center - *orig;
		float len = length(illuVec);
		illuVec = normalize(illuVec);
		if (len >= tItem || len >= tIllu) {
			continue;
		}
		float value;
		if ((value = dot(illuVec, *dir)) > 0.f) {
			glow += (float3){255, 255, 255} * RT_Data->illu[0].k * pow(value, 5000);
		}
	}
	return (float3){
		clamp(0.f, 255.f, glow.x),
		clamp(0.f, 255.f, glow.y),
		clamp(0.f, 255.f, glow.z)
	};
}
...
float3 Cast(struct RT_Data *RT_Data, float3 *orig,
	...
	if (true) {
		color += GlowCalc(RT_Data, orig, dir, tItem, tIllu);
	}
	if (tItem != INFINITY && (tItem < tIllu)) {
		if (obj != NULL) {*obj = RT_Data->item + itemIndex;}
		color += DiffLighting(RT_Data, orig, dir, tItem, itemIndex, node);
	...
	return (float3){
		...
		clamp(0.f, 255.f, color.y),
		clamp(0.f, 255.f, color.z)
	};
}
